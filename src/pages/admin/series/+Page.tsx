...
export default function AdminSeriesPage() {
  ...
  const openModal = (mode: 'add' | 'edit', s?: Series) => {
    ...
    setForm(s ? { ...s } : { slug: '', title: '', description: '', published: false });
    setModalOpen(true);
  };
  const closeModal = () => {
    setModalOpen(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // --- Zod validation ---
      const result = SeriesSchema.safeParse(form);
      if (!result.success) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '2rem auto', padding: '2rem', background: '#fff', borderRadius: 16, boxShadow: '0 4px 24px #0002', fontFamily: 'inherit' }}>
      <BackToAdmin />
      <h1 style={{ fontWeight: 700, fontSize: '2rem', marginBottom: 8, color: accent }}>Manage Series</h1>
      <button
        style={{
          marginBottom: 20,
          background: accent,
      ...
      {modalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: '#0008', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <form onSubmit={handleSubmit} style={{ background: '#fff', padding: 36, borderRadius: 14, minWidth: 350, maxWidth: 500, boxShadow: '0 4px 24px #0003', position: 'relative', fontFamily: 'inherit' }}>
            <h2 style={{ fontWeight: 700, fontSize: '1.4rem', marginBottom: 18, color: accent }}>{modalMode === 'add' ? 'Add Series' : 'Edit Series'}</h2>
            {/* Basic Info */}
            ...
            <fieldset style={{ border: '1px solid #e5e7eb', borderRadius: 8, marginBottom: 18, padding: 16 }}>
              ...
              <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>Checked: Series is visible on the public site. Unchecked: Series is hidden (draft).</div>
      ...
      {deleteSlug && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: '#0008', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', padding: 36, borderRadius: 14, minWidth: 300, boxShadow: '0 4px 24px #0003', textAlign: 'center', fontFamily: 'inherit' }}>
            <p style={{ fontWeight: 500, fontSize: '1.1rem', marginBottom: 18 }}>Are you sure you want to delete this series?</p>
