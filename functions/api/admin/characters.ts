...
export const onRequest: PagesFunction = async (context) => {
	...
	try {
		if (method === 'POST') {
			...
			const existing = await env.DB.prepare('SELECT id FROM characters WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
			if (existing) {
			...
			if (world_slug) {
				const world = await env.DB.prepare('SELECT slug FROM worlds WHERE slug = ? AND deleted_at IS NULL').bind(world_slug).first();
				if (!world) {
		...
		if (method === 'GET') {
			...
			if (slug) {
				const character = await env.DB.prepare('SELECT * FROM characters WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
				if (!character) {
					return new Response(JSON.stringify({ error: 'Not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
				}
				return new Response(JSON.stringify(character), { status: 200, headers: { 'Content-Type': 'application/json' } });
			} else {
				const rows = await env.DB.prepare('SELECT * FROM characters WHERE deleted_at IS NULL').all();
