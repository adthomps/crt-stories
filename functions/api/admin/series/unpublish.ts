...
export const onRequest: PagesFunction = async (context) => {
  ...
  try {
    ...
    const result = await env.CRT_STORIES_CONTENT.prepare(
      "UPDATE series SET published = 0, updated_at = datetime('now') WHERE slug = ? AND deleted_at IS NULL"
    ).bind(slug).run();
    if (result.changes === 0) {
    ...
    ).bind(adminEmail, 'unpublish', 'series', JSON.stringify({ slug })).run();
    return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
    console.error('Series unpublish API error:', err);
