// Cloudflare Pages Function: /api/admin/unpublish
import { z } from 'zod';
...
export const onRequest: PagesFunction = async (context) => {
	...
	const parse = UnpublishSchema.safeParse(body);
	if (!parse.success) {
	...
	const result = await env.CRT_STORIES_CONTENT.prepare(
		`UPDATE ${entity} SET published = 0, updated_at = datetime('now') WHERE slug = ? AND deleted_at IS NULL`
	).bind(slug).run();
	if (result.changes === 0) {
	...
	).bind(adminEmail, 'unpublish', entity, JSON.stringify({ slug })).run();
...
// Cloudflare Pages Function: /api/admin/unpublish
// Scaffold for unpublish endpoint (generic)
export {};
