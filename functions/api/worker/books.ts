	...
	try {
		if (method === 'GET') {
			...
			if (slug) {
			 ...
			 if (method === 'POST') {
				const { slug, title, description = '', coverImage = '', publishDate = '', badges = [], tags = [], formats = [], characterSlugs = [], world_slug = '', series_slug = '', excerpt = '', published = false } = body;
				 if (!slug || !title) {
				 ...
				 await env.CRT_STORIES_CONTENT.prepare('INSERT INTO books (slug, title, description, cover_image, publish_date, badges, tags, formats, characterSlugs, world_slug, series_slug, excerpt, published, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime("now"), datetime("now"))')
					 .bind(slug, title, description, coverImage, publishDate, JSON.stringify(badges), JSON.stringify(tags), JSON.stringify(formats), JSON.stringify(characterSlugs), world_slug, series_slug, excerpt, published ? 1 : 0).run();
			 ...
			 if (method === 'PUT') {
				const { slug, title, description = '', coverImage = '', publishDate = '', badges = [], tags = [], formats = [], characterSlugs = [], world_slug = '', series_slug = '', excerpt = '', published = false } = body;
				 ...
				 await env.CRT_STORIES_CONTENT.prepare('UPDATE books SET title = ?, description = ?, cover_image = ?, publish_date = ?, badges = ?, tags = ?, formats = ?, characterSlugs = ?, world_slug = ?, series_slug = ?, excerpt = ?, published = ?, updated_at = datetime("now") WHERE slug = ? AND deleted_at IS NULL')
					 .bind(title, description, coverImage, publishDate, JSON.stringify(badges), JSON.stringify(tags), JSON.stringify(formats), JSON.stringify(characterSlugs), world_slug, series_slug, excerpt, published ? 1 : 0, slug).run();
				 return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
			 }
			 if (method === 'DELETE') {
				 const { slug } = body;
